...
#include "MecsSensor.h"

/*****************************************************************************/

MecsSensor::MecsSensor() : 
	SensorBase(ECS_CTRL_DEV_NAME, "ecompass_data"),
	mEnabled(0),
	mPendingMask(0),
	mInputReader(32)
{
	...
	for (int i=0 ; i<numSensors ; i++)
		mDelays[i] = 200000000;	// 200 ms [ SENSOR_DELAY_NORMAL ]
	...
	if (!ioctl(dev_fd, ECOMPASS_IOC_GET_AFLAG, &flags)) {
		if (flags)  {
			...
			if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_ACCEL_STATUS), &absinfo)) {
				mPendingEvents[Accelerometer].acceleration.status = uint8_t(absinfo.value & SENSOR_STATE_MASK);
			}
		}
	}
	...
	if (!ioctl(dev_fd, ECOMPASS_IOC_GET_PFLAG, &flags)) {
		if (flags)  {
			mEnabled |= 1<<Proximity;
			if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_PROXIMITY), &absinfo)) {
				mPendingEvents[Proximity].distance = absinfo.value * CONVERT_DISTANCE;
			}
		}
	}

	if (!ioctl(dev_fd, ECOMPASS_IOC_GET_LFLAG, &flags)) {
		if (flags)  {
			mEnabled |= 1<<Light;
			if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_LIGHT), &absinfo)) {
				mPendingEvents[Light].light = absinfo.value * CONVERT_LIGHT;
			}
		}
	}
...
int MecsSensor::enable(int32_t handle, int en)
{
	int what = -1;
	switch (handle) {
	...
	if ((uint32_t(newState)<<what) != (mEnabled & (1<<what))) {
		...
		int cmd;
		switch (what) {
		case Accelerometer: cmd = ECOMPASS_IOC_SET_AFLAG; break;
		case MagneticField: cmd = ECOMPASS_IOC_SET_MFLAG; break;
		case Orientation:   cmd = ECOMPASS_IOC_SET_OFLAG; break;
		case Proximity:     cmd = ECOMPASS_IOC_SET_PFLAG; break;
		case Light:         cmd = ECOMPASS_IOC_SET_LFLAG; break;
		}
		short flags = newState;
		err = ioctl(dev_fd, cmd, &flags);
		err = err < 0 ? -errno : 0;
		LOGE_IF(err, "ECOMPASS_IOC_SET_XXX failed (%s)", strerror(-err));
		if (!err) {
			mEnabled &= ~(1<<what);
...
int MecsSensor::setDelay(int32_t handle, int64_t ns)
{
#if 1
	int what = -1;
	switch (handle) {
...
int MecsSensor::readEvents(sensors_event_t* data, int count)
{
	if (count < 1)
		return -EINVAL;
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			LOGE("MecsSensor: unknown event (type=%d, code=%d)",
				type, event->code);
			mInputReader.next();
		}
	}
...
void MecsSensor::processEvent(int code, int value)
{
	switch (code) {
		// Accelerometer:
		//
		...
		case EVENT_TYPE_PROXIMITY:
			...
			mPendingEvents[Proximity].distance = value * CONVERT_DISTANCE;
			break;
		...
		case EVENT_TYPE_LIGHT:
			...
			mPendingEvents[Light].light = value * CONVERT_LIGHT;
			break;
	}
}
